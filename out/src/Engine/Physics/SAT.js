class SAT {
    getOverlap(overlapVector, shapeAVertices, shapeBVertices, reverse) {
    ...
    getIntersection(shapeA, shapeB, intersectionAxis, intersectionDepth) {
        ...
        for (let i = 0; i < shapeANormals.length; i++) {
            ...
            let overlap = this.getOverlap(shapeANormals[i], shapeAVertices, shapeBVertices, reverse);
            if (overlap < 0.0) {
                return false;
            }
        ...
        for (let i = 0; i < shapeBNormals.length; i++) {
            let reverse = { value: false };
            let overlap = this.getOverlap(shapeBNormals[i], shapeAVertices, shapeBVertices, reverse);
            if (overlap < 0.0) {
                return false;
            }
            if (overlap < intersectionDepth.depth) {
